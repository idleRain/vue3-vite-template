...
- MSW mock 层：handler 级别开关，mock 与真实接口零代码切换——`config.ts` 是唯一配置源，改 `false` 即透传，HMR 即时生效
...
```
mocks/
├── config.ts       # 接口级开关 map，as const satisfies 保类型安全
├── helpers.ts      # guard(key, factory) — 读 config，决定 mock 或 bypass()
├── data/           # 响应体定义（返回什么）
├── handlers/       # 路由定义（哪个 URL 映射到哪个 data）
├── browser.ts      # setupWorker（开发模式）
└── server.ts       # setupServer（测试模式）
```

每个 handler 通过 `guard()` 接入开关：`config.ts` 中值为 `true` 时 MSW 拦截并返回 mock；值为 `false` 时 `bypass()` 把请求
透传到真实网络。ky 的 hook 管道（鉴权、业务码剥离、错误 toast）照常执行——MSW 在 `fetch` 层拦截，对 services 层完全透明。

测试与开发共用同一套 handler。`setup.ts` 在测试启动时强制启用所有 config，开发时关掉的 mock 不会悄悄搞垮测试。
